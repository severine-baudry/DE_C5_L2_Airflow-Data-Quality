...
def check_greater_than_zero(*args, **kwargs):
    ...
    redshift_hook = PostgresHook("redshift")
    # table is a user definer arugment, transmitted by setting the task parameter : param = {dict}
    ...
    logging.info(f"table {table} : len(records) = {len(records)}")
    logging.info(f"table {table} : len(records[0]) = {len(records[0])}")
